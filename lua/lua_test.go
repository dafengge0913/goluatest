...
func TestStackTrace(t *testing.T) {
	...
	err := L.DoFile("calls_test.lua")
	if err == nil {
		t.Fatal("No error returned from the execution of calls_test.lua")
	}

	le := err.(*LuaError)

	if le.Code() != LUA_ERRERR {
		t.Fatalf("Wrong kind of error encountered running calls_test.lua: %v (%d %d)\n", le, le.Code(), LUA_ERRERR)
	}
